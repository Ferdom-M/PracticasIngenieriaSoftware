...
C_Collision::C_Collision(Entity* _pOwner, vec2 _vPos, float _fRadius, unsigned int _uNumBalls, unsigned int _uCurrentBall, Ball* _tBalls)
	: Component(_pOwner)
	, m_vPos(_vPos)
	, m_fRadius(_fRadius)
...
void C_Collision::Slot(const double _dDeltaTime)
{
	...
	for (unsigned int j = 0; j < m_uNumBalls; j++) {
		if (m_uCurrentBall != j) {
			...
			vec2 vPosJ = m_tCCollision[j]->GetPos();
																	// recibir radio por find component collision
	...
	if (!collision) {
		// find component para poner en desplazamiento
